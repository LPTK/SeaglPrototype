...
object Parser extends TokenParsers {
  ...
  case class State(ind: Int, trailingOp: Bool = false)
  ...
  def lambdaBlock(ind: Int): Parser[Lambda] =
    (emptyLines ~> atIndent(ind) ~> "|" ~> space.? ~> lambdaBranch(State(ind))) ~ rep(newLine ~>
     emptyLines ~> atIndent(ind) ~> "|" ~> space.? ~> lambdaBranch(State(ind))) ^^ {
  ...
  def lambda(st: State): Parser[Lambda] =
    (/*air("|").? ~>*/ rep1sep(lambdaBranch(st), air("|")) ^^ Lambda) | indented(st, lambdaBlock, false)
  
  def lambdaBranch(st: State): Parser[(Term, Term)] = (term(st) <~ air("=>")) ~ term(st) ^^ {
    case a ~ b => (a,b)
  }
  ...
  def indented[T](st: State, p: Int => Parser[T], strict: Bool = true): Parser[T] = "indentedBlock" !!! (emptyLines ~> guard(space.?) ^? { // TODO better error on not defined
...
//  def parens()
  ...
  def subTerm(st: State): Parser[Term] = rep1(
    symbol ^^ Id
  | ("(" ~> space.? ~> genTerm(State(0, false)) <~ space.? <~ ")") // TODO ( + newline ...
  ...
  def genTerm(st: State) = rep1sep(lambda(st) | term(st), space.?) <~ space.? ^^ { _ reduceLeft App }
  
  def stmt(st: State): Parser[Stmt] = "stmt" !!! genTerm(st)
